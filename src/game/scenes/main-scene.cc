...
namespace Scenes
{
        ...
        : Scene::Scene(engine, window)
    {
        ...
        auto roomE  = ma.get_rooms().at(
                rand() % (ma.get_rooms().size() - 1) + 1);
            ...
            sf::RenderWindow& window, sf::Time& dt)
    {
        ...
        if (bonus_found != "")
        {
            gobject_delete(bonus_found);
            bonuses_.erase(std::remove(bonuses_.begin(), bonuses_.end(),
                    bonus_found), bonuses_.end());
        }
